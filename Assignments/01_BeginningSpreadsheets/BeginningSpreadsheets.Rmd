...
But I teach at the University of Maryland. And surprise! Head Coach Mike Locksley visits class. He makes $4 million. Per year.

The average salary now? $673,516. Who feels average now?
...
One thing you might want to do is compare your university's graduation rate compared to other schools like yours. For this example, [I've provided a CSV file of schools in the Big 10 athletic conference](https://raw.githubusercontent.com/dwillis/jour479x-exercises/master/Data/bigten_gradrates_2020.csv) that you should download (math alert: has 14 schools in it). If you'd like to get your own athletic conference, this data comes from the US federal Department of Educations [IPEDS](https://nces.ed.gov/ipeds/datacenter/Default.aspx) database.
...
So, from our last example, we know we can go to row 16 column C and work our AVERAGE magic.
...
And when we hit enter, we see that the Big 10 graduates 83.5 percent of students in six years. By eyeballing it, we can see some schools, like Northwestern, graduate nearly all of their students in six years. And, on the opposite end of the spectrum, we see the University of Nebraska-Lincoln graduating only 66 percent of students in six years.

So the first question that jumps out at you should be a very simple one: Why? Also, what's with Penn State?
...
So, with our college cost spreadsheet still open, let's compare the Average to the Median. Column D is the In State Cost and Column E is the Out of State Cost. Immediately underneath the last record, in row 16 and in column D, let's calculate the Average In State Cost. It's `=AVERAGE(D2:D15)` and you get \$32126.57 Under that, in Row 17 Column D, let's calculate the Median In State Cost. To do that, it's `=MEDIAN(D2:D15)` and you get \$28365.50 See that? The median is about \$4,000 cheaper than the average. You can do the same with Out of State costs, but the difference is less significant.

##### How to turn a median into a story

In 2004, at the dawn of what would become a economy-killing real estate bubble, Matt Waite was working at a newspaper in Florida, which was one of the key states in the whole mess. But at the time, people were just astonished at how fast homes were selling and for how much. So he set out to quantify it. And what we found was that neighborhood after neighborhood, street by street, the entire area was being transformed by skyrocketing home prices. The entire story was built on medians. He used geographic information systems -- maps -- to calculate the median sale prices of a home in a neighborhood in a given year and then calculated the same median home price in the current year, and did a percent change calculation.
...
Let's look at where students at the University of Maryland come from. Every year, the university performs a census of students across campus shortly after classes start. That census produces tons of data, but one of the most interesting is students by place of residence. Inside Maryland, that's the county (or Baltimore city). Outside, it's nearby states and other areas. 

Let's ask this question: what percentage of students come from New Jersey compared to Virginia? Compared to some Maryland counties?

##### How to calculate a percentage in a spreadsheet

Open the file -- [you can download it here](https://raw.githubusercontent.com/dwillis/jour479x-exercises/master/Data/umd_students.csv) -- and in Column D, Row 1, let's add a new header called Percent of Total.

Then, in Column D Row 2, let's calculate our percentage. So we want to take the subset (each jurisdiction) and divide it by the total (the row labeled Total). We need to anchor our total because it doesn't change, so our formula will look like this: `=C2/$C$36`
...
Percents of a whole end up in stories all over the place. In the case of our student data here, I'd want to know why. Why do New Jersey residents make up nearly 6% of the student body, compared to about 2% from Virginia? What's driving this? And (looking to the topic of the next section) how has this changed over time?
...
Let's try a new dataset. This one is called bigten_fbattendance.csv -- [download it here](https://raw.githubusercontent.com/dwillis/jour479x-exercises/master/Data/bigten_fbattendance.csv) -- and it is the total football attendance for Big Ten schools from 2014-2019.
...
When you hit enter, you'll have the percent change in attendance for Illinois.
...
And just like that, you've calculated the percent change for all schools.

##### How to turn a percent change into a story

Percent changes by themselves are rarely interesting, but what happens if we add a sort to this? Remember, we can sort two ways. What stories pop out when you sort for the fastest shrinking schools first? What happens when you sort for fastest growing? What happens when you calculate the change over a different time period?

## For the assignment

Answer the following questions: Which Big Ten school had the largest percentage change between 2014-2019? Which had the smallest? What does that calculation obscure? Put your answers below, add and commit this file to GitHub and submit the URL of this notebook (for example, mine is https://github.com/dwillis/jour479x-exercises/blob/master/Assignments/01_BeginningSpreadsheets/BeginningSpreadsheets.Rmd) in ELMS.
